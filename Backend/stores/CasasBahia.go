...
func getCasasBahiaRequest(url string) string {
	data := []byte(``)
	request, err := http.NewRequest("GET", url, bytes.NewBuffer(data))
	if err != nil {
		fmt.Println("CasasBahia [0] Error: ", err)
		return ""
	}
	...
	if err != nil {
		fmt.Println("CasasBahia [1] Error: ", err)
	} else {
...
type CasasBahiaProps struct {
	PageProps CasasBahiaPageProps `json:"pageProps"`
}

type CasasBahiaPageProps struct {
	InitialState CasasBahiaInitialState `json:"initialState"`
}

type CasasBahiaInitialState struct {
	Search Search `json:"search"`
}

type CasasBahiaSearch struct {
	Results Results `json:"results"`
}

type CasasBahiaResults struct {
	Size     int                  `json:"size"`
	Products []CasasBahiaProducts `json:"products"`
}

type CasasBahiaProducts struct {
	Id          string  `json:"id"`
...
type CasasBahiaRuntimeConfig struct {
	ResultsPerPage string `json:"RESULTS_PER_PAGE"`
	PriceApiKey    string `json:"PRICE_API_KEY"`
}

type CasasBahiaSearchResponse struct {
	Props         CasasBahiaProps         `json:"props"`
	RuntimeConfig CasasBahiaRuntimeConfig `json:"runtimeConfig"`
}

/*	*/

type CasasBahiaPrecoProdutos struct {
	PrecoVenda CasasBahiaPrecoVenda `json:"PrecoVenda"`
}

type CasasBahiaPrecoVenda struct {
	IdSku            int     `json:"IdSku"`
...
type CasasBahiaSearchResponsePrices struct {
	PrecoProdutos []CasasBahiaPrecoProdutos `json:"PrecoProdutos"`
}

func CasasBahiaSearchOnPage(query string, page int, lowPrice string, highPrice string) []types.Product {
	var productList []types.Product
	queryString := strings.ReplaceAll(fmt.Sprintf("https://www.casasbahia.com.br/%s/b?ordenacao=precoCrescente", query), " ", "-")
	queryStringPrice := "https://api.casasbahia.com.br/merchandising/oferta/v1/Preco/Produto/PrecoVenda/?idsProduto="
	...
	html := getCasasBahiaRequest(queryString)
	doc, err := goquery.NewDocumentFromReader(strings.NewReader(html))
	if err != nil {
		fmt.Println("CasasBahia [2] Error: ", err)
	}

	htmljson := string(doc.Find(`#__NEXT_DATA__`).First().Text())
	var searchResponse CasasBahiaSearchResponse
	err = json.Unmarshal([]byte(htmljson), &searchResponse)
	if err != nil {
		fmt.Println("CasasBahia [3] Error: ", err)
		return productList
	}
	...
	htmlPrices := getCasasBahiaRequest(queryStringPrice)
	var SearchResponsePrices CasasBahiaSearchResponsePrices
	err = json.Unmarshal([]byte(htmlPrices), &SearchResponsePrices)
	if err != nil {
		fmt.Println("CasasBahia [4] Error: ", err)
		return productList
	}
	...
	for _, result := range searchResponse.Props.PageProps.InitialState.Search.Results.Products {
		...
		if len(productPrice) > 0 {
			productList = append(productList, types.Product{Store: "CasasBahia", TotalPages: fmt.Sprint(totalPages), Image: result.Image, Name: result.Title, Link: result.Href, Stars: fmt.Sprintf("%f", result.Rating), StarsQty: fmt.Sprintf("%f", result.RatingCount), Price: productPrice})
		}
	}
	return productList
}
